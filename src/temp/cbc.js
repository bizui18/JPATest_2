...
(function(window){
    ...
    var My = {
      ...
      saveFile: async function() {
        ...
        _writable.write('</style>\n<head></head>\n<body>\n');
        _writable.write('<h1 id="okok">'+window.document.head.getElementsByTagName('title')[0]?.innerText + '</h1>');
        ...
        Object.keys(My.pidWordsMap).sort().forEach((value)=>{
      ...
      keyDown:function(target){
        target.addEventListener("keydown", (event) => {
          if(event.code=='Escape'){ // add to list
          ...
          }else if(event.code=='Space'){ // check stored list
            ...
            console.log('tt')
            Object.entries(My.pidWordsMap).forEach((t)=>{
              t[1] = My.mergeRange(t[1]);
            })
          }
